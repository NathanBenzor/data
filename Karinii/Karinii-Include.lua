...
Elemental_WS = S {'Earth Shot', 'Ice Shot', 'Water Shot', 'Fire Shot', 'Wind Shot', 'Thunder Shot', 'Gust Slash',
                  'Cyclone', 'Energy Steal', 'Energy Drain', 'Aeolian Edge', 'Burning Blade', 'Red Lotus Blade',
                  'Shining Blade', 'Seraph Blade', 'Sanguine Blade', 'Frostbite', 'Freezebite', 'Herculean Slash',
                  'Cloudsplitter', 'Primal Rend', 'Dark Harvest', 'Shadow of Death', 'Infernal Scythe',
                  'Thunder Thrust', 'Raiden Thrust', 'Blade: Teki', 'Blade: To', 'Blade: Chi', 'Blade: Ei', 'Blade: Yu',
                  'Tachi: Goten', 'Tachi: Kagero', 'Tachi: Jinpu', 'Tachi: Koki', 'Shining Strike', 'Seraph Strike',
                  'Flash Nova', 'Rock Crusher', 'Earth Crusher', 'Starburst', 'Sunburst', 'Cataclysm', 'Vidohunir',
                  'Garland of Bliss', 'Omniscience', 'Flaming Arrow', 'Hot Shot', 'Wildfire', 'Trueflight',
                  'Leaden Salute'}
...
Ready_TP = S {'Sic', 'Somersault', 'Dust Cloud', 'Foot Kick', 'Sheep Song', 'Sheep Charge', 'Lamb Chop', 'Rage',
              ...
              'Sudden Lunge', 'Noisome Powder', 'Beak Lunge', 'Suction', 'Drainkiss', 'Acid Mist', 'TP Drainkiss',
              'Back Heel', 'Jettatura', 'Choke Breath', 'Fantod', 'Charged Whisker', 'Purulent Ooze', 'Corrosive Ooze',
              'Tortoise Stomp', 'Harden Shell', 'Aqua Breath', 'Sensilla Blades', 'Tegmina Buffet', 'Zealous Snort',
...
do
    ...
    function precastequip(spell)
        ...
        if sets.Idle then
            built_set = set_combine(built_set, sets.Idle)
        end
        -- WeaponSkill
        if spell.type == 'WeaponSkill' then
            if sets.WS then
                ...
                if spell.skill == "Marksmanship" or spell.skill == "Archery" then
                    ...
                    if sets.WS[spell.english] then
                        ...
                        if state.OffenseMode.value ~= 'TP' and sets.WS[state.OffenseMode.value] and
                            sets.WS[state.OffenseMode.value].RA then
    ...
    function midcastequip(spell)
        ...
        if sets.Midcast then
            ...
            if sets.Midcast.SIRD and spell.action_type ~= 'Ranged Attack' then
                built_set = set_combine(built_set, sets.Midcast.SIRD)
            end
            -- Ranged Attack
            if spell.action_type == 'Ranged Attack' then
                if sets.Midcast.RA then
                    ...
                    -- Generic
                    local message = ''
                    if state.OffenseMode.value ~= 'TP' and sets.Midcast[state.OffenseMode.value] and
                        sets.sets.Midcast[state.OffenseMode.value].RA then
            ...
            elseif spell.type == 'BlackMagic' then
                ...
                elseif spell.skill == 'Elemental Magic' then
                    ...
                    if spell.target.id == last_skillchain_id and os.clock() - last_skillchain_time < 8 and
                        last_skillchain_elements[element_name] then
                        info("Burst Detected!")
        ...
        if spell.type == 'BardSong' and spell.target.type ~= 'MONSTER' then
            ...
            if spell.target.type ~= 'SELF' and spell.name ~= "Honor March" and spell.name ~= "Aria of Passion" and
                not SongCount:contains(spell.name) then
                if Instrument then
                    if Instrument.Pianissimo then
                        built_set = set_combine(built_set, {
                            range = Instrument.Pianissimo
                        })
                    else
        ...
        -- If TH mode is on - check if new mob and then equip TH gear
        if state.TreasureMode.value ~= 'None' and spell.target.type == 'MONSTER' and
            not th_info.tagged_mobs[spell.target.id] and sets.TreasureHunter then
            built_set = set_combine(built_set, sets.TreasureHunter)
            info('[' .. spell.english .. '] Set with Treasure Hunter')
        end
    ...
    function jobsetup(LockStylePallet, MacroBook, MacroSet)
        ...
        windower.send_command('wait 2;input /lockstyleset ' .. LockStylePallet .. ';wait 1;input /macro book ' ..
                                  MacroBook .. ';wait 1;input /macro set ' .. MacroSet ..
                                  ';gs validate;wait 2;input /echo Change Complete')
        ...
        if spell.name:contains('Cure') or spell.name:contains('Cura') then
            if world.weather_element == spell.element or spell.element == world.day_element then
                ...
                local Obi = player.inventory["Hachirin-no-Obi"] or player.wardrobe["Hachirin-no-Obi"] or
                                player.wardrobe2["Hachirin-no-Obi"] or player.wardrobe3["Hachirin-no-Obi"] or
                                player.wardrobe4["Hachirin-no-Obi"] or player.wardrobe5["Hachirin-no-Obi"] or
                                player.wardrobe6["Hachirin-no-Obi"] or player.wardrobe7["Hachirin-no-Obi"] or
                                player.wardrobe8["Hachirin-no-Obi"]
                local Staff = player.inventory["Chatoyant Staff"] or player.wardrobe["Chatoyant Staff"] or
                                  player.wardrobe2["Chatoyant Staff"] or player.wardrobe3["Chatoyant Staff"] or
                                  player.wardrobe4["Chatoyant Staff"] or player.wardrobe5["Chatoyant Staff"] or
                                  player.wardrobe6["Chatoyant Staff"] or player.wardrobe7["Chatoyant Staff"] or
            ...
            local Osash = player.inventory["Orpheus's Sash"] or player.wardrobe["Orpheus's Sash"] or
                              player.wardrobe2["Orpheus's Sash"] or player.wardrobe3["Orpheus's Sash"] or
                              player.wardrobe4["Orpheus's Sash"] or player.wardrobe5["Orpheus's Sash"] or
                              player.wardrobe6["Orpheus's Sash"] or player.wardrobe7["Orpheus's Sash"] or
    ...
    function main_engine()
        ...
        if not player or player.status == "Dead" or player.status == "Engaged dead" or buffactive['Charm'] or
            buffactive['Sleep'] then
            return
        end
        -- Status Ailment Check
        ...
        if player.status == "Engaged" then
            if sets.OffenseMode then
                ...
                if sets.OffenseMode[state.OffenseMode.value] then
                    ...
                    if buffactive['Aftermath: Lv.3'] and sets.OffenseMode.AM3 and
                        sets.OffenseMode.AM3[state.WeaponMode.value] then
                    ...
                    elseif buffactive['Aftermath'] and sets.OffenseMode.AM and
                        sets.OffenseMode.AM[state.WeaponMode.value] then
