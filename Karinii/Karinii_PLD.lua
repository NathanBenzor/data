...
state.WeaponMode:options('Sakpatas Sword', 'Burtgang', 'Naegling', 'Club', 'Shining One')
state.WeaponMode:set('Sakpatas Sword') -- Default weapon mode
...
function get_sets()
    ...
    sets.Enmity = { -- Goal is 200 total -Crusade is 30 and Burtang is 23
        ...
        legs = {
            ...
            augments = {'CHR+12', 'Pet: Mag. Acc.+5', 'Phalanx +4', 'Mag. Acc.+17 "Mag.Atk.Bns."+17'}
        },
        feet = {
    ...
    sets.Midcast.Enhancing = {
        ...
        legs = {
            name = "Founder's Hose",
            augments = {'MND+8', 'Mag. Acc.+14', 'Attack+13', 'Breath dmg. taken -3%'}
        },
        feet = "Sakpata's Leggings",
    ...
    sets.Midcast["Phalanx"] = set_combine(sets.Idle, sets.Midcast.SIRD, {
        ...
        back = {
            name = "Weard Mantle",
            augments = {'VIT+2', 'Enmity+2', 'Phalanx +2'}
        }
    })
    ...
    sets.Midcast.Blue_Magic = set_combine(sets.Idle, sets.Enmity, sets.Midcast.SIRD, {})

    sets.Midcast["Sheep Song"] = set_combine(sets.Idle, sets.Enmity, sets.Midcast.SIRD, {})
    sets.Midcast["Geist Wall"] = set_combine(sets.Idle, sets.Enmity, sets.Midcast.SIRD, {})
    sets.Midcast["Jettatura"] = set_combine(sets.Idle, sets.Enmity, sets.Midcast.SIRD, {})
    sets.Midcast["Blank Gaze"] = set_combine(sets.Idle, sets.Enmity, sets.Midcast.SIRD, {})
    sets.Midcast["Soporific"] = set_combine(sets.Idle, sets.Enmity, sets.Midcast.SIRD, {})

    sets.Cover = {
        body = {
            name = "Cab. Surcoat +3",
            augments = {'Enhances "Fealty" effect'}
        }
    }
    ...
    sets.JA["Invincible"] = set_combine(sets.Enmity, {
        legs = {
            name = "Cab. Breeches +3",
            augments = {'Enhances "Invincible" effect'}
        }
    })
    ...
    sets.JA["Sentinel"] = set_combine(sets.Enmity, {
        ...
        feet = {
            name = "Cab. Leggings +3",
            augments = {'Enhances "Guardian" effect'}
        }
    })
    ...
    sets.JA["Iron Will"] = set_combine(sets.Enmity, {
        head = {
            name = "Cab. Coronet +3",
            augments = {'Enhances "Iron Will" effect'}
        }
    ...
    sets.JA["Chivalry"] = set_combine(sets.Enmity, {
        hands = {
            name = "Cab. Gauntlets +3",
            augments = {'Enhances "Chivalry" effect'}
        }
...
function check_buff_JA()
    ...
    if os.clock() - buff_time > Buff_Delay then
        ...
        if player.sub_job == 'SAM' and player.sub_job_level > 24 then
            if not buffactive['Hasso'] and not buffactive['Seigan'] and ja_recasts[138] == 0 and player.sub_job_level >
                24 then
                buff = "Hasso"
            end
        end
    ...
    if os.clock() - tank_time > Tank_Delay then
        if (player.status == "Engaged" or windower.ffxi.get_player().target_locked) and state.JobMode.value == "ON" then
            ...
            elseif ja_recasts[46] == 0 and state.JobMode.value == "ON" and player.main_job_level > 14 then
                buff = "Shield Bash"
            elseif ja_recasts[159] == 0 and player.mp < 150 and player.tp > 2000 and state.JobMode.value == "ON" and
                player.main_job_level > 14 then
                buff = "Chivalry"
