...
do
        ...
        elseif spell.type == 'BardSong' then
            ...
            if not buffactive['Nightingale'] then
                ...
                if SongCount:contains(spell.name) then
                    equipSet = set_combine(equipSet, sets.Precast.FastCast, sets.Precast.Songs, {
                ...
                if SongCount:contains(spell.name) then
                    ...
                    equipSet = set_combine(sets.Midcast, sets.Midcast.DummySongs, {
                        range = Instrument.Count
                    })
                    -- Equip Marsyas
                elseif spell.name == "Honor March" then
                    equipSet = set_combine(sets.Midcast, equip_song_gear(spell), {
                        range = Instrument.Honor
                    })
                    -- Equip Loughnashade
                elseif spell.name == "Aria of Passion" then
    ...
    function midcastequip(spell)
        ...
        elseif spell.type == 'WhiteMagic' then
            ...
            elseif spell.name:contains('Regen') then
                if spell.target.type == 'SELF' then
                    ...
                    equipSet = set_combine(equipSet, sets.Midcast.SIRD, sets.Midcast.Enhancing.Others,
                        sets.Midcast.Regen)
                ...
                if spell.target.type == 'SELF' then
                    ...
                    elseif Enhancing_Skill:contains(spell.name) then
                        if buffactive['Accession'] then
                            equipSet =
                                set_combine(equipSet, sets.Midcast.Enhancing.Skill, sets.Midcast.Enhancing.Others)
        ...
        elseif spell.type == 'BardSong' and not buffactive['Nightingale'] then
            ...
            elseif SongCount:contains(spell.name) then
                ...
                -- Equip Marsyas
            elseif spell.name == "Honor March" then
                equipSet = set_combine(equipSet, equip_song_gear(spell), {
                    range = Instrument.Honor
                })
                -- Equip Loughnashade
            elseif spell.name == "Aria of Passion" then
    ...
    function do_Utsu_checks(spell)
        if spell.name == 'Utsusemi: Ichi' or spell.name == 'Utsusemi: Ni' or spell.name == 'Utsusemi: San' then
            ...
            local available_shihei = player.inventory['Shihei'] or player.inventory['Shikanofuda']
            local shihei_warning_level = 20
            if available_shihei.count < shihei_warning_level then
                local msg =
                    '*****  LOW SHIHEI WARNING: ' .. tostring(available_shihei.count) .. 'x on ' .. player.name ..
                        ' *****'
                local border = ""
                for i = 1, #msg do
                    border = border .. "*"
    ...
    function choose_set()
        ...
        -- Combat Checks
        if player.status == "Engaged" then
            equipSet = set_combine(equipSet, sets.OffenseMode, sets.OffenseMode[state.OffenseMode.value],
