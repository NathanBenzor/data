...
state.BurstMode = {}
state.BurstMode = M {
    ['description'] = 'Burst Specific Mode'
}
...
function display_box_update()
    ...
    for k, v in next, dialog do
        lines:insert(v.description ..
                         string.format('[%s]', tostring(v.value)):lpad(' ', width - string.len(tostring(v.description))))
    end
...
SlotList = {"main", "sub", "range", "ammo", "head", "body", "hands", "legs", "feet", "neck", "waist", "lear", "rear",
            "left_ring", "right_ring", "back"}

-- Song Sets
EnfeebleSong = S {'Foe Requiem', 'Foe Requiem II', 'Foe Requiem III', 'Foe Requiem IV', 'Foe Requiem V',
...
Storms = S {"Aurorastorm", "Voidstorm", "Firestorm", "Sandstorm", "Rainstorm", "Windstorm", "Hailstorm", "Thunderstorm",
            "Aurorastorm II", "Voidstorm II", "Firestorm II", "Sandstorm II", "Rainstorm II", "Windstorm II",
            "Hailstorm II", "Thunderstorm II"}

Enhancing_Skill = S {'Temper', 'Temper II', 'Enaero', 'Enstone', 'Enthunder', 'Enwater', 'Enfire', 'Boost-STR',
                     'Boost-DEX', 'Boost-VIT', 'Boost-AGI', 'Boost-INT', 'Boost-MND', 'Boost-CHR'}
...
EnfeeblingNinjitsu = S {'Jubaku: Ichi', 'Kurayami: Ni', 'Hojo: Ichi', 'Hojo: Ni', 'Kurayami: Ichi', 'Dokumori: Ichi',
...
geomancy = M('Geo-Acumen', 'Geo-Attunement', 'Geo-Barrier', 'Geo-STR', 'Geo-DEX', 'Geo-VIT', 'Geo-AGI', 'Geo-INT',
...
indicolure = M('Indi-Acumen', 'Indi-Attunement', 'Indi-Barrier', 'Indi-STR', 'Indi-DEX', 'Indi-VIT', 'Indi-AGI',
...
function pretargetcheck(spell, action)
    ...
    if buffactive['Stun'] then
        cancel_spell()
        equip(sets.Idle)
        return log('Cancel Spell - Player is stunned')
    end
    if buffactive['KO'] then
        cancel_spell()
        return log('Cancel Spell - Player is dead')
    end
    ...
    elseif spell.type == 'JobAbility' or spell.type == 'BloodPactWard' or spell.type == 'BloodPactRage' or spell.type ==
        'PetCommand' then
...
function precastequip(spell)
    ...
    elseif spell.type == 'WhiteMagic' then
        equipSet = sets.Precast
        if spell.name:contains('Raise') or spell.name == "Arise" or spell.name:contains('Cura') or
            spell.name:contains('Cure') or spell.name:contains('Reraise') then
    ...
    elseif spell.type == 'BardSong' then
        ...
        if not buffactive['Nightingale'] then
            ...
            if SongCount:contains(spell.name) then
                ...
                -- Equip Marsyas
            elseif spell.name == "Honor March" then
                equipSet = set_combine(sets.Midcast, {
                    range = Instrument.Honor
                })
                -- Equip Harp
...
function midcastequip(spell)
    ...
    -- Equip body
    if spell.name == "Impact" then
        equipSet = set_combine(equipSet, sets.Midcast, sets.Midcast.Nuke, equipSet[spell.english], {
            head = empty,
            body = "Crepuscular Cloak"
        })
        equip(equipSet) -- Test if availible
...
function choose_set()
    ...
    if player.status == "Engaged" then
        equipSet =
            set_combine(equipSet, sets.OffenseMode[state.OffenseMode.value], sets.Weapons[state.WeaponMode.value])
        if DualWield == false then
            if TwoHand == false then
                equipSet = set_combine(equipSet, sets.Weapons.Shield)
            end
...
function check_buff()
    ...
    if state.AutoBuff.value ~= 'OFF' and is_Busy == false and not areas.Cities:contains(world.area) and
        not buffactive['Stun'] and not buffactive['Terror'] then
        command_JA = 'None'
        command_SP = 'None'
        command_BP = 'None'
...
function use_enchantment(item)
    ...
    disable(slot)
    windower.send_command('wait ' .. item_table.cast_delay + 3 .. ';input /item "' .. item_table.en .. '" <me>;wait ' ..
                              item_table.cast_time - .5 .. ';gs enable ' .. slot)
end
...
function jobsetup(LockStylePallet, MacroBook, MacroSet)
    ...
    send_command('bind !` gs c burstmode')
...
function on_zone_change_for_th(new_zone, old_zone)
    Unlock()
    if settings.debug then
        add_to_chat(123, 'Zoning. Clearing tagged mobs table.')
    end
...
function cleanup_tagged_mobs()
    ...
    for target_id, action_time in pairs(th_info.tagged_mobs) do
        ...
        if time_since_last_action > 180 then
            ...
            if settings.debug then
                add_to_chat(123, 'Over 3 minutes since last action on mob ' .. target_id ..
...
windower.register_event('prerender', function()
    ...
    if now - UpdateTime2 > .25 then
        ...
        if position and not buffactive['Mounted'] and not buffactive['Sleep'] then
            local movement = math.sqrt((position.x - mov.x) ^ 2 + (position.y - mov.y) ^ 2 + (position.z - mov.z) ^ 2) >
                                 0.5
...
windower.register_event('action', function(data)
    ...
    if data ~= nil then
        ...
        if state.TreasureMode.value ~= 'None' then
            if data.actor_id == player.id and windower.ffxi.get_mob_by_id(data.targets[1].id).is_npc and
                TaggingCategories:contains(data.category) then
...
function Lock()
    disable('main', 'sub', 'range', 'ammo', 'head', 'neck', 'lear', 'rear', 'body', 'hands', 'lring', 'rring', 'waist',
        'legs', 'feet')
end
